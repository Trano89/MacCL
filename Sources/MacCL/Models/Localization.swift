...
enum L10n {
    ...
    private static let table: [String: (fr: String, en: String, de: String, pt: String, es: String)] = [
        ...
        "max_output_hint": ("Si une réponse dépasse ce plafond, le tour s'arrête avec « response exceeded the … output token maximum ». Montez-le pour les tâches longues ; le modèle et votre forfait imposent leur propre limite au-delà.", "If a reply exceeds this cap the turn stops with \"response exceeded the … output token maximum\". Raise it for long tasks; the model and your plan still impose their own ceiling.", "Überschreitet eine Antwort diese Grenze, endet der Zug mit „response exceeded the … output token maximum“. Für lange Aufgaben erhöhen; Modell und Tarif setzen darüber hinaus eigene Grenzen.", "Se uma resposta exceder este limite, o turno termina com \"response exceeded the … output token maximum\". Aumente-o para tarefas longas; o modelo e o seu plano impõem o seu próprio teto.", "Si una respuesta supera este límite, el turno se detiene con \"response exceeded the … output token maximum\". Súbelo para tareas largas; el modelo y tu plan imponen su propio techo."),
        ...
        "agent_new": ("Nouveau sous-agent", "New sub-agent", "Neuer Sub-Agent", "Novo subagente", "Nuevo subagente"),
        "agent_name": ("Nom", "Name", "Name", "Nome", "Nombre"),
        "agent_when": ("Quand l'utiliser", "When to use it", "Wann verwenden", "Quando usar", "Cuándo usarlo"),
        "agent_when_hint": ("Décrit au modèle principal quand déléguer. Sans description, l'agent n'est jamais choisi.", "Tells the main model when to delegate. With no description, the agent is never picked.", "Sagt dem Hauptmodell, wann delegiert wird. Ohne Beschreibung wird der Agent nie gewählt.", "Diz ao modelo principal quando delegar. Sem descrição, o agente nunca é escolhido.", "Indica al modelo principal cuándo delegar. Sin descripción, el agente nunca se elige."),
        "agent_model": ("Modèle", "Model", "Modell", "Modelo", "Modelo"),
        "agent_model_inherit": ("Celui de la conversation", "Same as the conversation", "Wie die Unterhaltung", "O da conversa", "El de la conversación"),
        "agent_machine": ("Machine", "Machine", "Maschine", "Máquina", "Máquina"),
        "agent_machine_same": ("Serveur de la conversation", "The conversation's server", "Server der Unterhaltung", "Servidor da conversa", "Servidor de la conversación"),
        "agent_prompt": ("Instructions de l'agent", "Agent instructions", "Anweisungen des Agenten", "Instruções do agente", "Instrucciones del agente"),
        "agent_delete_confirm": ("Supprimer ce sous-agent ?", "Delete this sub-agent?", "Diesen Sub-Agenten löschen?", "Eliminar este subagente?", "¿Eliminar este subagente?"),
        "agents_dir_note": ("Écrit dans %@", "Written to %@", "Gespeichert in %@", "Escrito em %@", "Escrito en %@"),
        ...
        "concurrency_hint": ("Le CLI en autorise 20 par défaut. Sur un seul serveur Ollama, chaque agent supplémentaire se bat pour la même mémoire.", "The CLI allows 20 by default. On a single Ollama server, each extra agent fights for the same memory.", "Das CLI erlaubt standardmäßig 20. Auf einem einzigen Ollama-Server konkurriert jeder weitere Agent um denselben Speicher.", "O CLI permite 20 por omissão. Num único servidor Ollama, cada agente extra disputa a mesma memória.", "El CLI permite 20 por defecto. En un solo servidor Ollama, cada agente extra compite por la misma memoria."),
